...
class SpotifyRecommender:
    def __init__(self):
        self.df = (pd.read_csv("ML-code/KMeans code/KMeans_data/cleaned_feature_data.csv")).to_numpy()
    ...
    def __genre_rec(self, artist):
        ...
        artist_genres = (self.df_artists.loc[self.df_artists['artists'] == artist]).iloc[0][15]
        artist_attribute = None # TODO (in nd_array)
        # Getting similar artists based on common genres.
        common_artists = {}
        for genre in artist_genres:
            genre_list = artist_genres.get(genre)
        ...
        return self.__euclidean_min(df_common_artists, artist_attribute, \
                                    [x for x in range(0, df_common_artists.count())], output_num=2)
            

    def __artist_rec(self, artist, spotifyQuery):
        df_artist_songs = (self.df_artists.loc[df_artists['artists'] == artist])
